...
6. メンテナが内容を確認し、マージします（**Squash merge のみ**。マージ後にトピックブランチは自動削除されます）

> `main` へのマージは、**CI がグリーンで、レビューコメントがすべて解決済み**であることが条件です。
> fork からの Pull Request はマージ権限を持たないため、メンテナが確認したうえでマージします。
