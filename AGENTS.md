...
**R11. CI を通さないマージ・ブランチ保護のバイパス禁止**

- Pull Request をマージする際、CI が失敗している状態でマージしたり、ブランチ保護（ruleset）を意図的にバイパスしてマージしないこと。
- 不合格のステータスや未解決のレビューコメントがある場合は、必ず根本原因を修正して解決すること。
- リポジトリ管理者はブランチ保護をバイパスできる設定になっているが、これは保護ルール自体が機能しなくなった場合の復旧手段であり、通常のマージでは使わないこと。
- ✅ 良い例: テストエラーが出た場合、ブランチに戻ってコード修正を行い、再度 CI がパスするのを確認してからマージする。
- ❌ ダメな例: PR の CI が赤いまま、管理者権限で「Merge without waiting for requirements to be met」を選んでマージする。
