...
> ⚠️ この構成では「設定 → メンバー管理」で発行される招待リンクが `http://localhost:3000/invite/...` になります。`localhost` は**リンクを開いた人自身のパソコン**を指すため、このリンクをメールやチャットで他の方へ送っても相手の環境では開けません。他の方を招待する場合は、**2-B. クラウドで運用する**（または他のパソコンからアクセスできる URL での公開）に切り替えてください。
...
**設定しなくてもメンバー招待は使えます。** 未設定の場合は、発行された招待リンクをコピーして、メールやチャットで自分で相手に共有してください（ただしローカル構成の `localhost` を含むリンクは相手のパソコンでは開けません。A-6 の注記を参照してください）。
