# OpenGreenTrack 機能仕様書

本書は OpenGreenTrack の**現行の機能仕様**（画面・列挙型・バリデーション規則・算定ロジック）を定義する。
...
OpenGreenTrack は、企業の温室効果ガス（GHG）排出量を算定・可視化・管理する Web アプリケーションである。Scope 1 / 2 / 3 を横断し、脱炭素経営の意思決定を支援する。
