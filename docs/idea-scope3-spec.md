...
本書は、AIST-IDEA（産総研 LCIデータベース。以下 IDEA）を利用した Scope 3 の活動量ベース算定（積上げ法）を OpenGreenTrack に実装するための設計書である。合意後に §7 の issue 分割に従って実装する。
...
- IDEA は有償の商用データベースであり、**OSS である OpenGreenTrack には同梱できない**。
- 利用者（OpenGreenTrack を導入した顧客）が **一般社団法人サステナブル経営推進機構（SuMPO）と自らライセンス契約**し、IDEA の Excel ファイルを入手して、自身の OpenGreenTrack インスタンスに**アプリ画面からアップロードして取り込む**（Bring Your Own License / Bring Your Own Data 方式）。
- したがって OpenGreenTrack 側に持つのは「**IDEA の Excel を解釈するインポーター**」と「**取り込んだ係数の保管・検索・算定への適用**」であり、**IDEA のデータそのものはリポジトリ・seed・テストコードに一切含めない**（テストは IDEA と同じ「形」のダミーデータで行う）。
