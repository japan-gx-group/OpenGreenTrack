# GreenTrack データベース設計仕様書

本書は、温室効果ガス（GHG）排出量算定・可視化ツール「GreenTrack」におけるデータベース（PostgreSQL）の物理設計、リレーションシップ、および各エンティティのデータ定義をまとめた設計書である。
...
GreenTrackは、機密性の高い企業の活動量データを取り扱うため、設計段階から以下のセキュリティ対策をデータベースレイヤーにおいて組み込んでいる。
